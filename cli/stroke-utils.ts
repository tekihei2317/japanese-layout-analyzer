...
export const makeFastStepper = (rules: Rule[]) => {
  ...
  rules.forEach((rule, index) => {
    for (let i = 1; i <= rule.input.length; i += 1) {
      ...
      const info = prefixMap.get(prefix) ?? { exactIndex: null, canGrow: false };
...
export const collectKeys = (rules: Rule[]) => {
  ...
  rules.forEach((rule) => {
    for (const ch of rule.input) keys.add(ch);
    if (rule.nextInput) {
      for (const ch of rule.nextInput) keys.add(ch);
    }
  });
