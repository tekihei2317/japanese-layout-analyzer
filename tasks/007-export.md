...
```text
bun run cli export --corpus data/texts/kenpou.txt
```

## exportコマンドの出力について

トップページで必要なデータと、テーブルページで必要なデータの両方が必要。

テーブルページで必要なデータは

打鍵効率、SFB(2-gram)、Scissors、SFB(3-gram)、SFS（3-gram）、ALT（3-gram）、ROLL（3-gram）、ONEHAND（3-gram）、REDIRECT(3-gram)、Hand use（左右の比）、In:out-roll（内と外の比）。

これは全部metrics.mdのデータから計算できる。

トップページで必要なデータは、どうやって表示するかは悩んでいるんだけど...。2-granの交互打鍵率は表示しなくていいかも。これだとトップページとテーブルページで表示するデータに差がなくなるから、テーブルページではサブカテゴリも表示する感じにすればいいかな。あと並び替えができるメリットがある。
